      ...
      <div class="flex flex-wrap mb-6 w-full">
        ...
        {{-- <div class="w-full md:w-1/2 px-3">
          ...
          <input class="appearance-none block w-full bg-gray-200 text-gray-700 border {{ $errors->has('type') ? 'border-red-500' : '' }}
            rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white"
            type="text" name="type" placeholder="Single" value="{{ old('type') }}">
      ...
      <div class="flex flex-wrap mb-6 w-full">
        ...
        <div class="w-full md:w-1/2 px-3">
          ...
          <div class="relative">
            <select class="block appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
              name="status">
              <option value="available" {{ old('status') == 'available' ? 'selected' : '' }}>Available for booking</option>
              <option value="occupied" {{ old('status') == 'occupied' ? 'selected' : '' }}>Currently occupied</option>
              <option value="booked"{{ old('status') == 'booked' ? 'selected' : '' }}>Room Booked</option>
            </select>
            <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-700">
              <svg class="fill-current h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M9.293 12.95l.707.707L15.657 8l-1.414-1.414L10 10.828 5.757 6.586 4.343 8z"/></svg>
            </div>
