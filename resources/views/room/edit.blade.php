...
<section class="text-gray-700 body-font">
  <div class="container px-5 py-12 mx-auto">
    <div class="flex flex-col text-center w-full mb-10">
      <h1 class="sm:text-4xl text-3xl font-medium title-font mb-2 text-gray-900">Edit Room</h1>
    </div>

    <div class="lg:w-2/3 w-full mx-auto overflow-auto">
        <form class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4"
        action="{{ route('rooms.update', $hotel) }}" method="POST">
          @csrf @method('PATCH')
          <div class="mb-4">
            <label class="block text-gray-700 text-sm font-semibold mb-2" for="name">Hotel Name</label>
            <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
              name="name" id="name" type="text" placeholder="name" value="{{ old('name', $hotel->name) }}">
          </div>
